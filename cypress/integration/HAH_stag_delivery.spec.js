describe("Testing Order", () => {
  it("Membuka situs", () => {
    ...
    cy.get(
      ".mt-3 > :nth-child(1) > .cursor-pointer > .flex > .text-xs"
    ).click();
    cy.get("#locationInput")
      .type("Indonesia")
      .should("have.value", "Indonesia");
    cy.wait(500);
    cy.get(".highlighted > .block").click();
    cy.get(".mt-4 > .px-3").click();

    // Summary Charge
    // cy.get('.pt-2 > .mb-2.text-red-dark').should('have.text', ' 2020-12-08 at 16:30 ')
    ...
    // cy.get('.mr-2 > :nth-child(1) > .border-b > .w-full').scrollIntoView({ duration: 1000 })
    ...
    // cy.get(':nth-child(4) > :nth-child(3) > div').should('have.text', '2020-12-08 at 16:30')
    ...
    // // Summary Charge
    // cy.get('.flex-col > .flex > .flex-auto').should('have.text', ' Happy Premium - 4 Menu X1(555฿)')
    // cy.get('.text-base').should('have.text', ' 555฿ ')
    // cy.get('.lg > .text-red-dark').should('have.text', '555฿')

    // // View Menu
    // cy.get('#landing-expand-menu-button').click()
    // cy.get('.mb-5 > .items-center > .font-black').should('have.text', ' Happy Premium - 4 Menu 555฿ x 1')
    // cy.get('.mb-5 > :nth-child(2) > .font-black').should('have.text', ' Group A (2) ')
    // cy.get('.mb-5 > :nth-child(2) > :nth-child(2) > :nth-child(1)').should('have.text', 'Shrimp Tempura (3 Pieces)')
    // cy.get(':nth-child(2) > :nth-child(2) > :nth-child(2)').should('have.text', 'X 1')
    // cy.get('.mb-5 > :nth-child(2) > :nth-child(3) > :nth-child(1)').should('have.text', 'Hot Tulip Wings (Deep Fried Chicken Wings) (4 Pieces)')
    // cy.get('.mb-5 > :nth-child(2) > :nth-child(3) > :nth-child(2)').should('have.text', 'X 1')
    // cy.get('.mb-5 > :nth-child(3) > .font-black').should('have.text', ' Group B (1) ')
    // cy.get('.mb-5 > :nth-child(3) > .flex > :nth-child(1)').should('have.text', 'BBQ Chicken Steak')
    // cy.get('.mb-5 > :nth-child(3) > .flex > :nth-child(2)').should('have.text', 'X 1')
    // cy.get('.mb-5 > :nth-child(4) > .font-black').should('have.text', ' Group C (1) ')
    // cy.get(':nth-child(4) > .flex > :nth-child(1)').should('have.text', 'Chocolate Lava with Raspberry Sauce')
    // cy.get(':nth-child(4) > .flex > :nth-child(2)').should('have.text', 'X 1')
    // cy.get('#landing-close-expanded-menu-button').click()

    // // Button Share dan Sign Up Now
    // cy.get(':nth-child(1) > .inline-block').should('have.text', ' Share Order ')
    // cy.get('#landing-signup-cta-button').should('have.text', ' Sign up now to earn 5 Hungry Points for this order ')
  });
});
